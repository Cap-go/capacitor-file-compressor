...
const SUPPORTED_OUTPUT_MIME_TYPES = ['image/jpeg', 'image/png', 'image/webp'] as const;

type SupportedOutputMimeType = (typeof SUPPORTED_OUTPUT_MIME_TYPES)[number];
...
export class FileCompressorWeb extends WebPlugin implements FileCompressorPlugin {
  ...
  async compressImage(options: CompressImageOptions): Promise<CompressImageResult> {
    ...
    const mimeType = (options.mimeType ?? 'image/jpeg').toLowerCase();

    if (!SUPPORTED_OUTPUT_MIME_TYPES.includes(mimeType as SupportedOutputMimeType)) {
      throw new Error(`Unsupported output mimeType: ${mimeType}. Supported: ${SUPPORTED_OUTPUT_MIME_TYPES.join(', ')}`);
    }
    ...
    const blob = await this.canvasToBlobWithinSize(
      canvas,
      mimeType as SupportedOutputMimeType,
      quality,
      options.blob.size,
    );
  ...
  private async canvasToBlobWithinSize(
    ...
    mimeType: SupportedOutputMimeType,
    quality: number,
    maxBytes: number,
  ): Promise<Blob> {
    ...
    while (true) {
      ...
      if (blob.size <= maxBytes || currentQuality <= this.minimumQuality || !supportsQuality) {
        return blob;
      }
  ...
  private canvasToBlob(canvas: HTMLCanvasElement, mimeType: SupportedOutputMimeType, quality: number): Promise<Blob> {
